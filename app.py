...
#	core_list = cores_id.split(",")
	...
	for i in product_list:
		if i == 'neighborhood_scalars' or i == 'mountain_top_scalars' or i == 'n_particles_scalars': 
		...
		if tablename in table_list_first_ele:
			...
			for row in rows:
				if product_type == 'blob':								#Checks datatype to determine how to append to dict
					...
					y[tablename]['products'].append(np.format_float_scientific(row[1], precision = 2, exp_digits = 2))
				else:
